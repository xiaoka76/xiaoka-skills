...
当用户需要生成逼真的直播带货截图风格视觉图时使用。此模板的核心优势在于将画面拆解为稳定结构，支持灵活指定主播来源（真人照片、名人名字、人物描述或完全随机生成），同时商品卡、聊天消息、背景品牌、礼物内容均可由用户指定、使用默认值或在合理范围内随机生成。
...
- **主播来源模式**：真人照片 / 名人名字 / 人物描述 / 随机生成——缺失会显著影响结果，必须优先确认
- **商品名称**：直播卖什么——核心信息，必须优先确认
- **平台风格**：抖音 / 小红书 / 淘宝直播 / 通用——决定 UI 视觉方向
- **界面语言**：中文还是英文——影响所有文字内容
...
当用户明确表示"你来补全"、"你随机生成"或"先给我一个 demo"时：

1. 只问最关键的 1-2 个问题（主播来源 + 商品信息）
2. 主播形象使用默认值或随机生成合理人设
3. 聊天消息、礼物弹窗等在合理范围内自动生成，风格与商品和平台一致
4. 不破坏主体一致性，不与用户已指定信息冲突
5. 不制造过于离谱的次要元素

## 避免事项

- UI 文字完全不可读——确保叠加界面文字清晰可辨
- 界面过度拥挤——各区域保持合理间距
- 人物脸部畸形——保持主播面部自然真实
- 商品卡透视错误——商品卡片角度与整体画面协调
- 聊天框与背景融在一起——聊天区域与背景有明确区分
- 主播与直播 UI 风格不协调——人物和界面应属于同一视觉层次
- 拼贴感过重——整体应像真实直播截图，而非简单元素堆叠