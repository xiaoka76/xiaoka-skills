...
function updatePromptEmptyState() {
  promptInput.classList.toggle("empty", getEditorPrompt().length === 0);
}
...
function activeAnnotationsInPrompt() {
  const activeIds = new Set(
    [...promptInput.querySelectorAll(".annotation-inline")].map((chip) => Number(chip.dataset.annotationId))
  );
  return state.annotations.filter((ann) => activeIds.has(ann.id));
}

function annotationIdsInPrompt() {
  return new Set([...promptInput.querySelectorAll(".annotation-inline")].map((chip) => Number(chip.dataset.annotationId)));
}
...
function selectionIsInsidePrompt() {
  const selection = window.getSelection();
  return selection.rangeCount > 0 && promptInput.contains(selection.getRangeAt(0).commonAncestorContainer);
}
...
function removePromptAnnotation(ann) {
  if (!ann) return;
  promptInput.querySelectorAll(".annotation-inline").forEach((chip) => {
...
promptInput.addEventListener("input", syncAnnotationsFromPrompt);
...
function makePreviewSection(title, className = "") {
  const section = document.createElement("div");
  section.className = `input-preview-section ${className}`.trim();
...
function renderModelInputPreview(modelInput) {
  resultEl.innerHTML = "";

  const wrapper = document.createElement("div");
  wrapper.className = "model-input-preview";

  const header = document.createElement("div");
  header.className = "model-input-header";
  const title = document.createElement("h3");
  title.textContent = "模型输入预览";
  const note = document.createElement("p");
  note.textContent = "学习版不会调用任何 API，这里展示点击生成时本来会提交给模型的输入。";
  header.appendChild(title);
  header.appendChild(note);
  wrapper.appendChild(header);

  const promptSection = makePreviewSection("Prompt");
  const promptBlock = document.createElement("pre");
  promptBlock.textContent = modelInput.prompt;
  promptSection.appendChild(promptBlock);
  wrapper.appendChild(promptSection);

  const imagesSection = makePreviewSection("Images");
  const imageGrid = document.createElement("div");
  imageGrid.className = "input-image-grid";
  for (const image of modelInput.images) {
    const card = document.createElement("div");
    card.className = "input-image-card";
    const img = document.createElement("img");
    img.src = image.dataUrl;
    img.alt = image.inputLabel;
    const caption = document.createElement("div");
    caption.className = "input-image-caption";
    caption.textContent = `${image.inputLabel} | ${image.name} | ${image.naturalWidth}x${image.naturalHeight}`;
    card.appendChild(img);
    card.appendChild(caption);
    imageGrid.appendChild(card);
  }
  if (!modelInput.images.length) {
    const empty = document.createElement("div");
    empty.className = "input-empty";
    empty.textContent = "当前 prompt 没有引用任何图片。";
...
function renderGeneratedImage(url) {
  const section = makePreviewSection("生成结果", "generated-section");
  const card = document.createElement("div");
...
generateBtn.addEventListener("click", async () => {
  ...
  if (!prompt) {
    setStatus("请输入编辑指令，或先用点击/框选模式生成 grounding。", true);
    return;
  }
  ...
  setStatus(`正在调用模型生成，共 ${modelInput.images.length} 张输入图...`);
  ...
  try {
    ...
    if (!data.url) {
      throw new Error("响应中没有 url 字段");
    }
    renderGeneratedImage(data.url);
    setStatus(`生成完成，模型：${data.model || "unknown"}。`);
  } catch (err) {
    console.error(err);
